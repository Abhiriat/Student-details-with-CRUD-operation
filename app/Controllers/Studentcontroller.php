...
public function create(){
   ...
   public function store(){
    ...
    $dta = $name . '-' . $email . '-' . $URN . '-' . $phone . '-' . $Regular.'-'.$course_code.'-'.$branch_code.'-'.$semester.'-'.$leet.'-'.$training;
   ...
   public function delete($name)
{
    ...
    if ($deleted) {
        ...
        return redirect()->to(base_url('students'))->with('status', 'Row deleted successfully');
    } else {
        // Redirect with error message
        return redirect()->to(base_url('students'))->with('status', 'No row found for deletion');
    }
}
public function edit($name)
    {
        ...
        if ($data['student']) {
            ...
            // Redirect with error message
            return redirect()->to(base_url('students'))->with('status', 'Student not found');
        }
    }
